...
void autonomous() {
  ...
  pros::delay(50);
    std::cout << "auto function" << "\n";
    if(autoSelector.get_value() > 2000){
      std::cout << "greater than 2000: " << autoSelector.get_value() << "\n";
    } else if(autoSelector.get_value() < 2000){
      std::cout << "less than 2000: "  << autoSelector.get_value() << "\n";
    } else {
      std::cout << "exception: " << autoSelector.get_value() << "\n";
    }
    /*
    while(1){
  ...
  if(autoSelector.get_value() > 2496 && autoSelector.get_value() < 3295){
    std::cout << "Red 2 Goal Auto" << "\n";
    std::cout << "this is the red 2 goal auto in the singular inside function" << "\n";
    xModel->xArcade(1, 0, -1);
  }
  if(autoSelector.get_value() > 1696 && autoSelector.get_value() < 2495){
    std::cout << "No Auto" << "\n";
    pros::delay(15000);
  }
  if(autoSelector.get_value() > 896 && autoSelector.get_value() < 1695){
    std::cout << "Blue 2 Goal Auto" << "\n";
    xModel->xArcade(-1, 0, 1);
  }
  if(autoSelector.get_value() > 0 && autoSelector.get_value() < 895){
